...
第一次世界大战期间，阿加莎•克里斯蒂成了一名志愿者。战争结束后，她创作了自己的第一部侦探小说《斯泰尔斯庄园奇案》。几经周折，作品于一九二○正式出版，由此开启了克里斯蒂辉煌的创作生涯。一九二六年，《罗杰疑案》由哈珀柯林斯出版公司出版。这部作品一举奠定了阿加莎•克里斯蒂在侦探文学领域不可撼动的地位。之后，她又陆续出版了《东方快车谋杀案》、《ABC 谋杀案》、《尼罗河上的惨案》、《无人生还》、《阳光下的罪恶》等脍炙人口的作品。时至今日，这些作品依然是世界侦探文学宝库里最宝贵的财富。根据她的小说改编而成的...`,
	...
	err = elasticSave(expected, "douban_book", client)
	if err != nil {
		t.Fatalf("Failed: error %v\n", err)
	}
	t.Logf("Save succeed, save id: %s\n", expected.Id)
	// get save item
	if err != nil {
		t.Fatalf("Failed: error %v\n", err)
	}
	resp, err := client.Get().Id(expected.Id).Index("douban_book").Do(context.Background())
	if err != nil {
		t.Fatalf("Failed: error %v\n", err)
	}
	defer func() {
		_, err = client.Delete().Index("douban_book").Id(expected.Id).Do(context.Background())
		if err != nil {
